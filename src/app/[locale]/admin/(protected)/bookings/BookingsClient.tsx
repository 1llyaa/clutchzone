  ...
  return (
    <div style={{ padding: '40px 48px' }}>
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 40 }}>
        <div>
          ...
          <p className="font-mono text-cz-gray-mid" style={{ fontSize: 11, letterSpacing: 2, marginTop: 4 }}>
            {bookings.length} REZERVACÍ PRO ZVOLENÝ DEN
          </p>
        </div>
        <input
          type="date"
        ...
        <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(10, 1fr)', marginBottom: 16 }}>
          {pcStations.map((s) => {
            ...
            return (
              ...
              >
                <span className="font-mono text-white" style={{ fontSize: 11, letterSpacing: 1 }}>
                  {s.label}
                </span>
                <span
                  className="font-mono uppercase"
                  style={{ fontSize: 8, letterSpacing: 1, marginTop: 3, color: state === 'occupied' ? '#E84A1A' : '#555' }}
                >
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: '1px solid #2A2A2A' }}>
              {['REFERENCE', 'ZÁKAZNÍK', 'KONTAKT', 'STANICE', 'ČAS', 'DÉLKA', 'CELKEM', 'STATUS', ''].map((h) => (
                <th
                  key={h}
                  className="font-mono text-cz-gray-mid uppercase text-left"
                  style={{ padding: '12px 14px', fontSize: 10, letterSpacing: 2 }}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {bookings.length === 0 ? (
              <tr>
                <td
                  colSpan={9}
                  className="font-mono text-cz-gray-mid text-center"
              ...
              bookings.map((b) => (
                ...
                >
                  <td className="font-mono text-cz-orange" style={{ padding: '12px 14px', fontSize: 12 }}>
                    {b.reference}
                  </td>
                  <td className="font-body text-white" style={{ padding: '12px 14px', fontSize: 13 }}>
                    {b.customer_name}
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                    <div className="font-mono text-cz-gray-light" style={{ fontSize: 11 }}>
                      {b.customer_email}
                    </div>
                    {b.customer_phone && (
                      <div className="font-mono text-cz-gray-mid" style={{ fontSize: 10, marginTop: 2 }}>
                        {b.customer_phone}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="font-mono text-white" style={{ padding: '12px 14px', fontSize: 12 }}>
                    {b.start_time?.slice(0, 5)}
                  </td>
                  <td className="font-mono text-cz-gray-light" style={{ padding: '12px 14px', fontSize: 12 }}>
                    {Math.round(b.duration_minutes / 60)}h
                  </td>
                  <td className="font-body text-white" style={{ padding: '12px 14px', fontSize: 13 }}>
                    {b.total_price} Kč
                  </td>
      ...
      {selected && (
        ...
        >
          <div
            className="bg-cz-black-mid h-full flex flex-col"
            style={{ width: 400, borderLeft: '1px solid #2A2A2A' }}
          ...
          >
            ...
            >
              <div>
                <div className="font-mono text-cz-orange" style={{ fontSize: 13 }}>
                  {selected.reference}
                </div>
              ...
              <button
                onClick={() => setSelected(null)}
                className="font-mono text-cz-gray-mid hover:text-white"
                style={{ fontSize: 18 }}
              >
                ×
              </button>
            ...
            <div className="flex-1 overflow-auto" style={{ padding: 28 }}>
              {[
                ...
                <div key={label} style={{ marginBottom: 16 }}>
                  <div className="font-mono text-cz-gray-mid uppercase" style={{ fontSize: 10, letterSpacing: 2, marginBottom: 4 }}>
                    {label}
                  </div>
            ...
            <div
              ...
              style={{ padding: '20px 28px', borderTop: '1px solid #2A2A2A' }}
            >
