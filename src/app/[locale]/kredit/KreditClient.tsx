          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
              {tiersByType(tab).map((tier) => {
                ...
                return (
                  <div key={tier.id} style={{ background: '#111111', border: `1px solid ${qty > 0 ? '#E84A1A' : '#2A2A2A'}`, borderRadius: 2, padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 12 }}>
                      ...
                      <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 12, letterSpacing: 1.5, color: '#E8E8E8' }}>KČ</span>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginTop: 20, borderTop: '1px solid #2A2A2A', paddingTop: 16 }}>
                      <button onClick={() => setQty(tab, tier.hours, qty - 1)} style={{ width: 40, height: 40, border: '1px solid #2A2A2A', background: '#1A1A1A', color: '#FFFFFF', fontSize: 18, cursor: 'pointer', borderRadius: 2 }}>−</button>
          ...
          <div style={{ position: 'sticky', top: 88, background: '#111111', border: '1px solid #2A2A2A', borderTop: '2px solid #E84A1A', padding: 28 }}>
            ...
            {cartLines.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {cartLines.map((l) => (
                  <div key={cartKey(l.stationType, l.hours)} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontFamily: "'Space Mono',monospace", fontSize: 13, letterSpacing: 1, color: '#E8E8E8', textTransform: 'uppercase' }}>
            ...
            <div
              ...
              style={{ display: 'flex', alignItems: 'flex-start', gap: 14, minHeight: 44, padding: '12px 14px', marginTop: 10, borderRadius: 2, border: `1px solid ${consentError ? '#E84A1A' : '#2A2A2A'}`, background: '#0A0A0A', cursor: 'pointer' }}
            >
              <div style={{ width: 20, height: 20, flexShrink: 0, marginTop: 2, border: `1.5px solid ${termsAccepted || consentError ? '#E84A1A' : '#555555'}`, background: termsAccepted ? '#E84A1A' : 'transparent', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#fff' }}>
                {termsAccepted ? '✓' : ''}
              </div>
            ...
            <button
              ...
              style={{
                width: '100%', height: 56, marginTop: 14, background: valid && !loading ? '#E84A1A' : '#2A2A2A', border: `1.5px solid ${valid && !loading ? '#E84A1A' : '#2A2A2A'}`,
